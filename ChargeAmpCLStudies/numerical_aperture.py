...
def confidence_plot(position, max_NAv, min_NAv, range_pickle = "pickle_files/zr_range_pickle.pickle", bound_pickle = "pickle_files/full_data_NAv_pickle.pickle", recalculate = True):
    ...
    plt.clf()
    fig, ax = plt.subplots()
    ax.plot(positions, upper_depth_scan)
    ax.fill_between(positions, abs_lower_depth_scan_results, abs_upper_depth_scan_results, color = 'grey', alpha = .2)
    ax.fill_between(positions, upper_depth_scan, lower_depth_scan, color = 'b', alpha = .2)

    ax.set_xlabel("Position (m)")
    ax.set_ylabel("Number of carriers")
    fig.savefig("plots/depth_scan_cl_NA.png")
    plt.clf()
