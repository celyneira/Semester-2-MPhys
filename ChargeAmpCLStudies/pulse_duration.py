...
CHARGE_DATA_FILE_NAME = "200_x3y4.txt"

SIC_DEPTH_SCAN = "200V/Zscan_SiC_x3y4_-200V.mat"
...
def direct_charge_density_integrand(z, r, V, beta2, Ep, tau):
    l = WAVELENGTH
...
def reflected_charge_density_integrand(z, r, V, beta2, Ep, tau):
    l = WAVELENGTH
    NAv = EFFECTIVE_NUMERICAL_APERTURE
    nr = REFRACTIVE_INDEX
    h = H
...
def interference_charge_density_integrand(z, r, V, beta2, Ep, tau):
    l = WAVELENGTH
    NAv = EFFECTIVE_NUMERICAL_APERTURE
    nr = REFRACTIVE_INDEX
    h = H
...
def array_charge_density_normalisation_with_reflection_and_smearing(V, beta, ep, tau):
    r_lim = R_LIMIT
    nr = REFRACTIVE_INDEX
    ...
    direct, error = integrate.dblquad(direct_charge_density_integrand, 0, r_lim, 0, z_lim, args=(V, beta, ep, tau))
    reflected, error = integrate.dblquad(reflected_charge_density_integrand, 0, r_lim, 0, z_lim, args=(V, beta, ep,tau))
    interference, error = integrate.dblquad(interference_charge_density_integrand, 0, r_lim, 0, z_lim, args=(V, beta, ep, tau))
...
def parallel_charge_density(V_values, beta, ep, tau):
    """Parallel computation of charge densities for multiple V values."""

    with mp.Pool(mp.cpu_count()) as pool:
        results = pool.starmap(array_charge_density_normalisation_with_reflection_and_smearing, [(V, beta, ep, tau) for V in V_values])
...
def fit_function(V, beta, ep, tau):
    result = parallel_charge_density(V, beta,ep, tau)
...
def charge_density_normalisation_with_reflection_and_smearing(V, beta, ep, tau):
    r_lim = R_LIMIT
    nr = REFRACTIVE_INDEX
    REF_INDEX_CORRECTION = 1/np.sqrt((nr**2-EFFECTIVE_NUMERICAL_APERTURE**2)/(1-EFFECTIVE_NUMERICAL_APERTURE**2))
...
def data_file_read(detector_thickness, recalculate_exp_charges=False, plot = False, plot_sim = False, debug = False, file = SIC_DEPTH_SCAN, filename = CHARGE_DATA_FILE_NAME, beta = BETA_2, tau = PULSE_DURATION, ep = PULSE_ENERGY):
    f = h5py.File(file, 'r')
...
def confidence_plot(position, tau_max, tau_min, tau_med,range_pickle = "pickle_files/tau_range_pickle.pickle", bound_pickle = "pickle_files/full_data_tau_pickle.pickle", recalculate = True):
    if __name__ == "__main__":
        ...
        plt.savefig("plots/depth_scan_tau_cl.png")
        plt.clf()
